...
import com.leq.singlepart.autofindwaybin.FangKuaiPosition;
import com.leq.singlepart.autofindwaybin.MyPanel;
...
public class AutoFindWay {
	...
	public static void main(String[] args) {
		...
		System.out.println("wayList=" + wayList.size());
		for (FangKuaiPosition fPosition : wayList) {
			System.out.println("fPosition=" + fPosition.getX() + "," + fPosition.getX() + "," + fPosition.getF());
		}
	}
	...
	public List<FangKuaiPosition> getWayLine(MyPanel cat, MyPanel fish) {
		...
		for (int i = 0; i < BasePanel.openList.size(); i++) {
			...
			/**
			 * 将中心方块的周围方块添加到openlist集合 注意：如果openlist中已经存在，则 需要将消耗最少的方块更新到 openlist中
			 */
		...
		/**
		 * 从 closedlist中获取到路线 先获取终点，然后根据fangkuaiposition.previousFk获取上一个方块，一直获取到起点
		 */
	...
	/**
	 * 获取周围方块 ①判断是否超越边界 ②判断是否是障碍物/已计算过的方块
	 * 
	 * @param fk 中心方块
	 * @return 周围方块结集合
	 */
